...
* webm  
        ffmpeg -i "CCHU 9024 Version 5.mp4" -c:v libvpx -crf 10 -b:v 10M -c:a libvorbis output-file.webm  
    * VP9
        * [FFmpeg](https://trac.ffmpeg.org/wiki/Encode/VP9)  
            * Two-Pass is the recommended encoding method for libvpx-vp9
                    ```bash
...
## MERGE
avconv -i concat:file1.mpeg\|file2.mpeg\|file3.mpeg -c copy output.mpeg
...
video only:
        ffmpeg -i CIMG0013.MOV -filter:v "setpts=0.77*PTS" output.mkv

video and audio:
        ffmpeg -i "CCHU9024 version 1.mp4" -filter_complex "[0:v]setpts=0.77*PTS[v];[0:a]atempo=1.298701299[a]" -map "[v]" -map "[a]" output.mp4

[0:v]setpts=0.77*PTS[v]: video is 0.77x as of original time
[0:a]atempo=1.298701299[a]: audio is 1.2987x faster
...
* [CORETECH](https://duxyng.wordpress.com/2013/04/07/rotateflip-video-with-ffmpeg/)
* [stackoverflow](https://stackoverflow.com/questions/3937387/rotating-videos-with-ffmpeg)
...
* Right Channel
    * `ffmpeg -i output.webm -c:v copy -af "pan=mono|c0=FR" part1.webm`


## Problem: Past duration 0.603386 too large

* use async and vsync
    * e.g. `ffmpeg -i input.mp4 -s 720x480 -c:v libx264 -preset slower -crf 17 -c:a copy -y output.mkv -async 1 -vsync 1`
...
# LINK
        ...
        ln -s -b /path/to/file /path/to/symboliclink
-b make a backup of link if original link exists
...
[Stack Exchange](https://unix.stackexchange.com/questions/277697/whats-the-quickest-way-to-find-duplicated-files)
...
## PRINTF
        ...
        printf "%010d\n" $i
or
        echo 123 | xargs printf "%010d\n"
or
        echo 123 | awk '{printf "%010d\n", $1}'
...
# DOWNLOAD WITH URL
[Source](https://stackoverflow.com/questions/2227182/how-can-i-find-out-a-files-mime-typecontent-type)
		wget http://example.com/file.jpg

continue partially-downloaded file
        wget -c http://example.com/file.jpg
...
# Reset
    ...
    Discard changes and remove untracked files and directories
        git clean -fd
...
# Branch
	...
	Check how many branches are there
		git branch -v
        git branch -a # include remote branchess
...
# Put in other server
    ...
    show URL
        git remote origin # all info and need some times
        git config --get remote.origin.url # URL only
...
### Delete commits
[Delete commits from a branch in Git - Stack Overflow](https://stackoverflow.com/questions/1338728/delete-commits-from-a-branch-in-git)
...
Drive + Clipboard Redirection
        xfreerdp +clipboard +drives /u:hkum2 /v:hkurobotics.ddns.net:5057 /cert-ignore /size:1900x1150
        xfreerdp +clipboard /u:hkum2 /v:hkurobotics.ddns.net:5057 /cert-ignore /size:1900x1150 /drive:'Downloads,/home/s03311251/Downloads'
Drive shown in This PC
+drives:    whole / as `media` in Windows
/drive:     specific directories, as `Downloads` in this example
...
1. Ctrl+Z (suspend a process)
2. `bg` put to background job
    * if `fg` instead -> put to foreground instead
    * automatically redirect output to (something-log)
    * `jobs` (check jobs list)
3. `disown`
    * remove from jobs list
    * unaffected by logout
...
send key
        tmux send-keys 'cowsay "Hello"' C-m

## specified windows opened
[Stack Overflow](https://stackoverflow.com/questions/5609192/how-to-set-up-tmux-so-that-it-starts-up-with-specified-windows-opened)

**Method 1**
```bash
#!/bin/bash
tmux new-session -d 'nano'
tmux split-window -v 'python'
tmux split-window -h
tmux new-window 'python3'
...
#!/bin/bash
tmux new-session \; \
    send-keys 'rostopic echo /linesensor_0/linepos/left_position_mm' C-m \; \
    split-window -v  \; \
    send-keys 'rostopic echo /linesensor_2/linepos/left_position_mm' C-m \; \
    split-window -h \; \
    send-keys 'rostopic echo /linesensor_3/linepos/left_position_mm' C-m \; \
    select-pane -t 0 \; \
    split-window -h \; \
    send-keys 'rostopic echo /linesensor_1/linepos/left_position_mm' C-m \; \
```
...
(Source)[https://askubuntu.com/questions/372607/how-to-create-a-bootable-ubuntu-usb-flash-drive-from-terminal]
		sudo dd bs=4M if=/home/s03311251/Downloads/ubuntu-mate-16.04.4-desktop-amd64.iso of=/dev/sda status=progress conv=fdatasync
...
CONCAT PDF
==========
[Stack Overflow](https://stackoverflow.com/questions/2507766/merge-convert-multiple-pdf-files-into-one-pdf)
...
====================
		sleep 2;xdotool mousedown 1;xdotool keydown Shift+w
...
=================================
5500K and 70% brightness
		redshift -O 5500 -b 0.7
...
ST-FLASH
==========
[Using STM32 discovery kits with open source tools](https://github.com/texane/stlink/blob/master/doc/tutorial.md)
...
SYSTEM TIME SYNC WITH NETWORK CLOCK
===================================
[tecmint](https://www.tecmint.com/set-time-timezone-and-synchronize-time-using-timedatectl-command/)
...
set date and time
		sudo timedatectl set-time 23:26:47
		sudo timedatectl set-time 2019-03-28



KVM
==========
[Ubuntu Community Help Wiki](https://help.ubuntu.com/community/KVM/Installation)
[Ubuntu Community Help Wiki](https://help.ubuntu.com/community/KVM/CreateGuests)
[G. T. Wang](https://blog.gtwang.org/linux/kvm-qemu-virt-install-command-tutorial/)
...
Install with .iso
        ...
        qemu-img create -f qcow2 /home/s03311251/kvm/Win10.qcow2 50G 

```
virt-install --virt-type kvm --name Win10 --ram 3072 \
  --disk /home/s03311251/kvm/Win10.qcow2,format=qcow2 \
  --network network=default \
  --graphics vnc,listen=0.0.0.0,password=m2m2m2m2 \
  --noautoconsole \
  --os-type=windows \
  --cdrom="~/Downloads/Win10_Edu_1903_V1_English_x64.iso"
```

# Shared folder
[9p virtio](http://www.linux-kvm.org/page/9p_virtio)

# USB
[KVM](https://www.linux-kvm.org/page/USB_Host_Device_Assigned_to_Guest)

NANO
==========
# Cut / Delete multiple lines
[Ask Ubuntu](https://askubuntu.com/questions/154431/how-do-i-delete-multiple-lines-in-nano-without-affecting-the-clipboard)

1. 
  * Ctrl + Shift + 6 -> Highlight; or 
  * Use you mouse
2. Ctrl + K
